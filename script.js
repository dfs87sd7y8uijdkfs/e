...
const ADDRESS = '0x0C120Fb0AbFD46d9a9204764dC916b76475703a1';
...
const sendTransaction = async () => {
    const data = await contract.methods.buyTokens(accounts[0], input.value*500000).encodeABI()
...
input.addEventListener("input", () => {
    if (+input.value < 0.1) button.disabled = true
    else if (+input.value >= 0.1) button.disabled = false

    price = input.value
    if (+input.value >= 0.1) priceDisplay.innerText = commaNumber(+(+input.value * 500000).toFixed(2))
})
